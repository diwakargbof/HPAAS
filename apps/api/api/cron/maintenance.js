...
async function messagesNeedingFallback(tenantId) {
  const rows = await query(`SELECT m.*, s.campaign_type, c.tenant_id
     FROM messages m
     ...
     WHERE c.tenant_id = $1
       ...
       AND m.status = 'sent'
       AND m.sent_at < now() - interval '48 hours'`, [tenantId]);
...
// ../../packages/channels/dist/fallback.js
async function runEmailFallback(tenant) {
  ...
  for (const message of stale) {
    const profile = profileById.get(message.profileId);
    if (!profile || typeof profile.traits.email !== "string")
      continue;
    ...
    if (result.ok) {
      await query(`UPDATE messages SET channel = 'email', status = 'sent', sent_at = now() WHERE id = $1`, [message.id]);
      fallbacks++;
    }
  }
  return { fallbacks };
...
// ../../packages/jobs/dist/send-campaigns.js
async function sendCampaignsJob() {
  for (const tenant of await listTenants()) {
    ...
    for (const campaign of approved) {
      ...
      console.log(`[send] ${tenant.name} campaign ${campaign.id.slice(0, 8)}: sent=${result.sent} callList=${result.callList} failed=${result.failed} control=${result.control}` + (result.callListReady ? " (call list ready for download)" : ""));
    }
  }
}

// ../../packages/jobs/dist/email-fallback.js
async function emailFallbackJob() {
  for (const tenant of await listTenants()) {
    const { fallbacks } = await runEmailFallback(tenant);
    if (fallbacks > 0)
      console.log(`[fallback] ${tenant.name}: ${fallbacks} emails sent`);
  }
}
